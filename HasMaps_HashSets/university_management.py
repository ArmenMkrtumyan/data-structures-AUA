...
class University:
    ...
    def enable_mentorship(self, mentor: Student, mentee: Student) -> bool:
        index = 0
        mentor = mentor._id #hasing mentor
        for i in self.university_map._hash_table:
            index += 1
            if i:
                if i._key == mentor:
                    tmp = i
                    ...
                    while tmp._next:
                        tmp = tmp._next
                    tmp._next = self.university_map.Entry(mentor, mentee, None)
                    return True
        if mentee == self.university_map.put(mentor, mentee):
            return True

    def get_mentor(self, mentee: Student)->Student:
        for i in self.university_map._hash_table:
            if i:
                tmp = i
                while tmp:
